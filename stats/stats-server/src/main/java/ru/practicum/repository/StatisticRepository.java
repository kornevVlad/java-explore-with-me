...
public interface StatisticRepository extends JpaRepository<Hit, Long> {
    ...
    @Query(value = "SELECT new ru.practicum.modelpackage.Stats(hits.app, hits.uri, COUNT(hits.ip)) "
            + "FROM Hit AS hits "
    ...
    @Query(value = "SELECT new ru.practicum.modelpackage.Stats(hits.app, hits.uri, COUNT(DISTINCT hits.ip)) "
            + "FROM Hit AS hits "
